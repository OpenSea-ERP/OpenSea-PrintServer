...
describe('isValidIncomingMessage', () => {
  ...
  it('rejeita print sem printerId', () => {
    expect(isValidIncomingMessage({
      type: 'print',
      jobId: 'j',
      data: 'd',
...
describe('mapWindowsStatusWithPnp', () => {
  it('USB com PnpStatus OK → ready', () => {
    expect(mapWindowsStatusWithPnp({
      PrinterStatus: 3,
      WorkOffline: false,
      PortName: 'USB001',
      PnpStatus: 'OK',
...
describe('classifyWindowsPrinterType', () => {
  it('PDF no nome → virtual', () => {
    expect(classifyWindowsPrinterType('Microsoft Print to PDF', 'PORTPROMPT:')).toBe('virtual');
  });

  it('XPS no nome → virtual', () => {
    expect(classifyWindowsPrinterType('Microsoft XPS Document Writer', 'PORTPROMPT:')).toBe('virtual');
  });

  it('OneNote no nome → virtual', () => {
    expect(classifyWindowsPrinterType('Send to OneNote 2016', 'nul:')).toBe('virtual');
  });
  ...
  it('porta TCP → network', () => {
    expect(classifyWindowsPrinterType('HP LaserJet', 'TCP_192.168.1.100')).toBe('network');
  });

  it('porta IP_ → network', () => {
    expect(classifyWindowsPrinterType('Epson L3150', 'IP_192.168.1.50')).toBe('network');
  });

  it('porta UNC → network', () => {
    expect(classifyWindowsPrinterType('Shared Printer', '\\\\server\\printer')).toBe('network');
  });
  ...
  it('porta LPT → local', () => {
    expect(classifyWindowsPrinterType('Generic Printer', 'LPT1:')).toBe('local');
  });

  it('"Print to" no nome → virtual', () => {
    expect(classifyWindowsPrinterType('Print to File', 'FILE:')).toBe('virtual');
  });
});
