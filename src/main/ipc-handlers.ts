...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle("agent:pair", async (_event, code: string) => {
    ...
    try {
      ...
      log.info(
        `[ipc] Agente pareado: ${safeLog(data.agentName)} (${safeLog(data.agentId, 32)})`,
      );
    ...
    try {
      ...
      if (agentId) {
        ...
        try {
          await apiRequest(
            apiUrl,
            `/v1/sales/print-agents/${encodeURIComponent(agentId)}/unpair`,
            {
              method: "POST",
              timeoutMs: 10_000,
            },
  ...
  ipcMain.handle(
    "printers:cancel-job",
    async (_event, printerName: string, jobId: number) => {
